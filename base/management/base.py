...
class CeleryCommand(BaseCommand):
    ...
    skip_opts = ['--app', '--loader', '--configs', '--no-color']
    requires_system_checks = False
    keep_base_opts = False
    stdout, stderr = sys.stdout, sys.stderr

    def get_version(self):
        return 'celery {c.__version__}'.format(
            c=celery
        )
    ...
    def set_broker(self, broker):
        os.environ[str_to_bytes("CELERY_BROKER_URL")] = str_to_bytes(broker)
    ...
    def handle_default_options(self, argv):
        ...
        for i, arg in enumerate(argv):
            ...
            if '--settings=' in arg:
                ...
                os.environ[str_to_bytes("DJANGO_SETTINGS_MODULE")] = str_to_bytes(settings_module)
