...
from bootstrap import celery_app as app
...
class Command(CeleryCommand):
    ...
    if hasattr(base, "preload_options"):
        preload_options = list(base.preload_options) if base.preload_options else []
    else:
        preload_options = []
    options = []
    options.extend(cc_options)
    options.extend(base_options)
    options.extend(preload_options)
