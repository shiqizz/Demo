...
def get_root_path():
    return os.path.dirname(os.path.dirname(bootstrap.__file__))
...
class ConfigInputStream(object):
    ...
    def __init__(self, stream):
        ...
        self.stream.write(stream.read().decode('utf-8').encode('utf-8').decode())
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            raise ConfigResolutionError(
                '%r is not a valid index for %r' % (index, object.__getattribute__(self, 'path')))
    ...
    def getToken(self):
        ...
        while True:
            ...
            if not c:
                ...
                try:
                    self.comment = self.comment.decode()
                except:
                    pass
                self.lineno += 1
                continue
